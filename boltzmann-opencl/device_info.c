...
int output_platform_info(cl_platform_id platform_id) {
    cl_char platform_name[1024] = {0};    // string to hold name of the platform
    cl_char vendor_name[1024] = {0};      // string to hold the name of the platform vendor
    cl_int err;

    err = clGetPlatformInfo(platform_id, CL_PLATFORM_NAME, sizeof(platform_name), platform_name, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to access platform name! %s\n", err_code_str(err));
        return EXIT_FAILURE;
    }

    err = clGetPlatformInfo(platform_id, CL_PLATFORM_VENDOR, sizeof(platform_name), vendor_name, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to access platform vendor name!\n");
        return EXIT_FAILURE;
    }

    printf("OpenCL platform %s from %s\n", platform_name, vendor_name);
...
int output_device_info(cl_device_id device_id) {
    ...
    printf(" with a maximum of %d compute units.\n", comp_units);
